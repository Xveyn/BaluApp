...
- **`AppNotification.category` is a lossy, display-only view of `rawCategory`, not the source of truth — the two are not interchangeable.** `rawCategory: String` is exactly what the server sent and is an open set (core categories, `"lifecycle"`, plugin names); `category: NotificationCategory` is a closed 8-value enum that exists only so the UI has a fixed, finite thing to switch on for an icon/label. `NotificationDto.toDomain()` (below) maps any `rawCategory` that doesn't case-insensitively match one of those 8 enum names — `"lifecycle"` and every plugin category included — to `NotificationCategory.SYSTEM` silently, with no signal anywhere that the mapping missed. Anything that filters, compares, or otherwise needs to distinguish notifications by category — `NotificationsViewModel.setCategory`/`selectedCategory` are `String?`, not `NotificationCategory?`, for exactly this reason — must use `rawCategory`; `category == NotificationCategory.SYSTEM` does not mean the notification's real category is `"system"`, only that it wasn't one of the other 7 names.
