...
- **A separate `MutableSharedFlow<String>(extraBufferCapacity = 1)` for snackbar events is the minority pattern, not the default.** Only 4 of the ~24 screen ViewModels use it — `DashboardViewModel`, `PowerDetailViewModel`, `AlwaysAwakeViewModel`, `FritzBoxSettingsViewModel` — plus `FolderSyncViewModel`, which uses a bufferless `MutableSharedFlow<String>()` (no `extraBufferCapacity`) because it emits a running stream of sync-progress messages, not one-shot events, so buffering the latest one is meaningless. The other ~19 ViewModels (e.g. `SettingsViewModel`, `FilesViewModel`) put a plain `error: String?` field directly on their `UiState` instead of using an event channel at all. Check the ViewModel's constructor/fields before assuming which shape a given screen follows.
