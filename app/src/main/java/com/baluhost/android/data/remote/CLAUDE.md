...
- **A notification's trash state is one nullable timestamp, and the two list endpoints are separate.** `NotificationDto.deletedAt` (`deleted_at`) is null while the notification is active and set once it is in the trash — there is no boolean dismissed flag and no query parameter that folds trashed rows into the normal list. `GET /notifications` returns active rows only, `GET /notifications/trash` returns trashed ones, and the four state-changing endpoints (`{id}/read`, `{id}/dismiss`, `{id}/restore`, `{id}/snooze`) each answer with the full updated `NotificationDto` — which is what `NotificationRepositoryImpl.sync()` relies on to know the server's state *after* it pushed, rather than writing back the pre-push snapshot it fetched. `DELETE {id}` and `DELETE /trash` return no body.
...
| `NotificationsApi.kt` | `/notifications` | List, unread count, read/dismiss/snooze, preferences, WebSocket token, plus the trash side: `getTrash`, `restore`, `deletePermanently`, `emptyTrash` |
